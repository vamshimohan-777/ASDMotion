"""Micro genetic NAS controller for constrained architecture search."""
...
class MicroGeneticNAS:
    ...
    def _random_population(self, search_space):
        return [NASCandidate(architecture=sample_architecture(search_space, self.rng)) for _ in range(self.population_size)]

    def _evaluate(self, population, evaluate_fn):
        for cand in population:
            if cand.metrics is None:
                cand.metrics = evaluate_fn(copy.deepcopy(cand.architecture))
                cand.fitness = compute_fitness(cand.metrics)
    ...
    def _crossover(self, arch_a, arch_b):
        ...
        for section in child.keys():
            ...
            for key in keys:
                child[section][key] = copy.deepcopy(
                    arch_a.get(section, {}).get(key) if self.rng.random() < 0.5 else arch_b.get(section, {}).get(key)
                )
        return child
    ...
    def _mutate(self, arch, search_space):
        ...
        d_model = int(out["encoder"].get("embedding_dim", 256))
        head_choices = [int(h) for h in search_space["transformer"]["heads"] if d_model % int(h) == 0]
        if not head_choices:
            head_choices = [2]
        if d_model % int(out["transformer"]["heads"]) != 0:
    ...
    def evolve(self, evaluate_fn, search_space=None, on_generation_end=None):
        ...
        for gen in range(1, self.generations + 1):
            ...
            while len(next_pop) < self.population_size:
                ...
                child_arch = self._mutate(self._crossover(p1.architecture, p2.architecture), search_space)
